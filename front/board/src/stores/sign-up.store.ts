// sign-in.store.ts
//% Java Class 또는 React의 컴포넌트의 파일명은 UpperCamelCase를 따랐다.
//% Typescript의 경우 특별한 파일의 네이밍 규칙이 지정되어있지 않기 때문에
//% 필수적으로 UpperCamelCase를 사용할 필요는 없다.
...
//^ 3 set메서드 : (파라미터) => set((...state) => ({파라미터}))
//^ : == [..., set 메서드]

//% spread 연산자
//? conste {요소1, 요소2, ...} = 객체;
//? ...객체 : 객체에서 지정한 요소를 제외하고 남은 요소를 객체로 묶음 처리 한다.
//? const {요소1, ...묶음 객체명} = 객체;
...
//? a만 따로 빼서 파괴 / 남은 b,c,d는 subObj가 하나로 묶어 가지게 된다.
//?     => subObj = { b:1, c:2, d:3 };
