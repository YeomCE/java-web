import { Box, Typography, Button, TextField, FormControl, InputLabel, InputAdornment, IconButton, Input } from '@mui/material';
...
function FirstPage() {
    ...
    const { setEmail, setPasswordCheck, setPassword } = useSignUpStore();
...
function SecondPage() {
    ...
    return (
        <Box>
            ...
            <TextField sx={{ mt: "40px" }} fullWidth label="휴대폰번호*" variant='standard' value={telNumber} onChange={(event) => setTelNumber(event.target.value)} />
...
export default function SighUpCardView({ setLoginView }: Props) {
    ...
    return (
        <Box
            ...
            <Box>
                {page === 1 && (<Button fullWidth variant='contained' size='large' sx={{ mb: "20px" }} onClick={onNextButtonHandler}>다음단계</Button>)}
                {page === 2 && (
                    <Button
